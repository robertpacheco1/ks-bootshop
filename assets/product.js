...
const initProductGallery = () => {
  ...
  window.addEventListener(
    "variantChange.ks.product",
    (event) => {
      const selectedVariant = event.detail;

      if (selectedVariant.featured_media) {
        bootstrap.Carousel.getOrCreateInstance("#product-gallery").to(
          selectedVariant.featured_media.position - 1
        );
      }
    },
    false
  ...
  if (window.matchMedia("(min-width: 992px)").matches) {
    const navbarHeight =
      document.querySelector("#shopify-section-navbar.sticky-top")
        .clientHeight || 0;
